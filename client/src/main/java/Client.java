...
public class Client {
	...
	public String[] signup(String user, String pass) {
		sendMessageToServer(ProtocolTags.SIGN_UP_TAG, user, pass);
		return this.getResponse();
	}
	
	public String[] signin(String user, String pass){
		sendMessageToServer(ProtocolTags.SIGN_IN_TAG, user, pass);
		return this.getResponse();
	}

	public void get_message(User toUser, int offset) {
		sendMessageToServer(
				ProtocolTags.GET_MESSAGE_TAG,
				String.valueOf(toUser.getUserId()),
				String.valueOf(offset));
	}
	
	public void send_message(User toUser, String msg) {
		sendMessageToServer(ProtocolTags.SEND_MESSAGE_TAG, String.valueOf(toUser.getUserId()), msg);
	}

	public void show_online_users() {
		sendMessageToServer(ProtocolTags.SHOW_ONLINE_USERS_TAG);
	}

	public void book_slot(User restaurantUser, String date, String time){
		sendMessageToServer(ProtocolTags.BOOK_SLOT_TAG, String.valueOf(restaurantUser.getUserId()),date,time);
	}

	public void show_bookings(User toUser) {
		sendMessageToServer(ProtocolTags.SHOW_BOOKINGS_TAG,String.valueOf(toUser.getUserId()));
	}

	public void show_open_slots(User restaurantUser, String date){
		sendMessageToServer(ProtocolTags.SHOW_OPEN_SLOTS_TAG,String.valueOf(restaurantUser.getUserId()),date);
	}
