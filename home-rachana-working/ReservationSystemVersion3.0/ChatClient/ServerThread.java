...
public class ServerThread extends Thread {
  ...
  private void handleShowOnlineUsers(String[] response) {
    ...
    for (int i = 1; i < response.length; i = i + User.FIELD_COUNT) {
      User currentUser = new User(
          ...
          response[i + 1],
          UserType.valueOf(response[i + 2]));
      chatroomModel.addOnlineUser(currentUser);
      if (currentUser.getUserType() == UserType.human) {
        addToModel(model, response[i + 1]);
      } else {
        addToModel(model, response[i + 1] + " - Restaurant");
      }
    }
  }
  ...
  private void displayBookingsServerResponse(DefaultListModel<String> model, String string) {
    addToModel(model, String.format("@%s: %s", chatroomModel.getToUser().getUsername(), string));
  }
}
